...
template<class T>
class mpriority_queue : public priority_queue<T, vector<T>, less<vector<T>::value_type> > {
public:
...
public:
	...
	CSimpleThreadPool(unsigned int nThreadsCount, unsigned int nQueueCapacity = 16): m_qMutex(), m_PQueue() {
		int i;
		CThread *thTask = NULL;
